...
def _url_join(*parts: object) -> str:
    """
    Join path parts using POSIX '/' separators.
    Works for both local-like relative paths and fsspec/HF URLs such as
    ...
    for i, p in enumerate(parts):
        ...
        # Preserve scheme part like 'hf://'
        if i == 0 and "://" in s:
            scheme, rest = s.split("://", 1)
            rest = rest.strip("/")
            if rest:
                cleaned.append(f"{scheme}://{rest}")
            else:
                cleaned.append(f"{scheme}://")
        else:
...
class WellDataset(Dataset):
    ...
    def __init__(
        ...
        if path is not None:
            self.data_path = str(path).replace("\\", "/")
            trunk_path = self.data_path
            if normalization_path is None:
                normalization_path = "stats.yaml"
            if well_split_name is not None:
                self.data_path = _url_join(path, "data", well_split_name)

        else:
            assert is_dataset_in_the_well(
                well_dataset_name
            ), f"Dataset name {well_dataset_name} not in the expected list {WELL_DATASETS}."
            self.data_path = _url_join(
                well_base_path, well_dataset_name, "data", well_split_name
            )
            trunk_path = _url_join(well_base_path, well_dataset_name)
            if normalization_path is None:
                normalization_path = _url_join(
                    well_base_path, well_dataset_name, "stats.yaml"
                )

        normalization_path = str(normalization_path).replace("\\", "/")
        trunk_path = str(trunk_path).replace("\\", "/")

        # For local absolute paths, keep them as-is. For URLs, join with POSIX separators.
        if "://" not in normalization_path and os.path.isabs(normalization_path):
            self.normalization_path = normalization_path
        else:
            rel_norm = str(normalization_path).removeprefix(str(trunk_path)).lstrip("./")
            self.normalization_path = _url_join(trunk_path, rel_norm)
